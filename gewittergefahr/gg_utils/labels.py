...
def _remove_data_near_end_of_tracking_period(
        storm_to_events_table, max_lead_time_sec):
    """Removes storm objects near end of tracking period.
    ...
    :return: storm_to_events_table: Same as input, but maybe with fewer rows.
    """
    ...
    storm_to_events_table.drop(
        storm_to_events_table.index[invalid_rows], axis=0, inplace=True)
...
def label_wind_speed_for_regression(
    ...
    :return: storm_to_winds_table: Same as input, with the following exceptions.
        [1] May have fewer rows (storm objects near the end of the tracking
            period are removed).
        [2] Contains new column with regression labels (column name generated by
            `get_column_name_for_regression_label`).
        [3] Contains new column with number of wind observations used to label
    ...
    storm_to_winds_table = _remove_data_near_end_of_tracking_period(
...
def label_wind_speed_for_classification(
    ...
    :return: storm_to_winds_table: Same as input, with the following exceptions.
        ...
        [3] Contains new column with number of wind observations used to label
            each storm object (column name generated by
            `get_column_name_for_num_wind_obs`).
        [4] Contains new column "num_wind_observations", indicating how many
...
def label_tornado_occurrence(
    ...
    storm_to_tornadoes_table = _remove_data_near_end_of_tracking_period(
