...
def column_name_to_num_classes(column_name, include_dead_storms=False):
    """Parses number of classes from column name.

    :param column_name: Column name (generated by either
        `get_column_name_for_regression_label` or
        `get_column_name_for_classification_label`).
    :param include_dead_storms: Boolean flag.  If column name involves wind
        speed and `include_dead_storms` = True, `num_classes` will be
        incremented by 1 (to account for the "dead storm" label).
    :return: num_classes: Number of classes.
    """

    parameter_dict = column_name_to_label_params(column_name)
    event_type_string = parameter_dict[EVENT_TYPE_KEY]
    if event_type_string == events2storms.TORNADO_EVENT_TYPE_STRING:
        return 2

    error_checking.assert_is_boolean(include_dead_storms)
    wind_speed_cutoffs_kt = parameter_dict[WIND_SPEED_CLASS_CUTOFFS_KEY]
