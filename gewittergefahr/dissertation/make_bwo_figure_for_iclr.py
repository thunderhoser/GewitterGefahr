...
MAIN_FILE_ARG_NAME = 'main_bwo_file_name'
UNCONSTRAINED_FILE_ARG_NAME = 'unconstrained_bwo_file_name'
OUTPUT_DIR_ARG_NAME = 'output_dir_name'

MAIN_FILE_HELP_STRING = (
    'Path to file with main backwards-optimization results (will be read by '
    '`backwards_optimization.read_file`).'
)
UNCONSTRAINED_FILE_HELP_STRING = (
    'Path to file with unconstrained backwards-optimization results (will be '
    'read by `backwards_optimization.read_file`).'
)
...
INPUT_ARG_PARSER.add_argument(
    '--' + MAIN_FILE_ARG_NAME, type=str, required=True,
    help=MAIN_FILE_HELP_STRING
)
INPUT_ARG_PARSER.add_argument(
    '--' + UNCONSTRAINED_FILE_ARG_NAME, type=str, required=True,
    help=UNCONSTRAINED_FILE_HELP_STRING
)
INPUT_ARG_PARSER.add_argument(
...
def _run(main_bwo_file_name, unconstrained_bwo_file_name, output_dir_name):
    ...
    panel_file_names[2] = _write_radar_figures(
        figure_objects=handle_dict[plot_examples.RADAR_FIGURES_KEY],
        field_names=radar_field_names, composite_name='after_unconstrained',
...
if __name__ == '__main__':
    ...
    _run(
        main_bwo_file_name=getattr(INPUT_ARG_OBJECT, MAIN_FILE_ARG_NAME),
        unconstrained_bwo_file_name=getattr(
            INPUT_ARG_OBJECT, UNCONSTRAINED_FILE_ARG_NAME
        ),
