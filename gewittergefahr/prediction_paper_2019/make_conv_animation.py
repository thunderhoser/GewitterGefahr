...
FIGURE_CAPTION = (
    'Schematic for 2-D convolution.\n'
    '[a-c] Input grids, each containing one variable.\n'
    '[d-f] Convolution filters, one for each input variable.  The '
    'weights in each filter are\n'
    'set manually and known to be useful for edge detection.  Inside a CNN, '
    'these\n'
    'weights are learned (adjusted by training).\n'
    '[g] The output grid (feature map) generated by convolution.  At each '
    'filter position,\n'
    'the highlighted value in panel g is the sum of elementwise products of '
    'highlighted\n'
    'values in panels a-c with highlighted values in panels d-f.'
)
