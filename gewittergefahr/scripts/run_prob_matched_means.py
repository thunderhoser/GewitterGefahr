...
SALIENCY_FILE_HELP_STRING = (
    ...
    ' want to composite class-activation maps instead, leave this argument '
    'alone.')

GRADCAM_FILE_HELP_STRING = (
    'Path to Grad-CAM file (will be read by `gradcam.read_file`).  If you want '
    'to composite saliency maps instead of class-activation maps, leave this '
    'argument alone.')
...
OUTPUT_FILE_HELP_STRING = (
    ...
    'written by `saliency_maps.write_file`.  If run on class-activation maps, '
    'output file will be written by `gradcam.write_file`.  To overwrite the '
    'input file, leave this empty.')
...
def _run(saliency_file_name, gradcam_file_name, max_percentile_level,
    ...
    if saliency_file_name in ['', 'None']:
        saliency_file_name = None
    if gradcam_file_name in ['', 'None']:
        gradcam_file_name = None
    
    # if output_file_name in ['', 'None']:
    #     if saliency_file_name is None:
    #         output_file_name = gradcam_file_name
    #     else:
    #         output_file_name = saliency_file_name
    ...
    if saliency_file_name is None:
        ...
        ) = saliency_maps.read_file(saliency_file_name)
